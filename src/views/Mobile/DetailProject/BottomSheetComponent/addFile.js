...
export default function AddFileComponent(props) {
    ...
    const handleAddFile = async (data, id) => {
        setLoading(true);
        if (status === 'Survey') {
            const response = await PROJECT_ADD_FILE_SURVEY(data, id);
        ...
        if (status === 'Approved Instalation') {
            const response = await PROJECT_ADD_FILE_INSTALATION(data, id);
            if (response.status === 400) {
                await snackBarOpen(response.data.error.message, 'error');
            }
            if (response.status === 200) {
        ...
        if (status === 'Termination') {
            const response = await PROJECT_ADD_FILE_TERMINATION(data, id);
            if (response.status === 400) {
                await snackBarOpen(response.data.error.message, 'error');
            }
            if (response.status === 200) {
        ...
        if (status === 'Labeling') {
            const response = await PROJECT_ADD_FILE_LABELING(data, id);
            if (response.status === 400) {
                await snackBarOpen(response.data.error.message, 'error');
            }
            if (response.status === 200) {
